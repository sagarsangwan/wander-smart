        ...
        <CardContent>
          <div className="flex flex-wrap gap-4 text-sm text-muted-foreground">
            ...
            <div className="flex items-center">
              ...
              <Button
                className="ms-3 "
                onClick={() => generateStyledPDF(tripData)}
              >
                Download PDF
              </Button>
            </div>
          ...
          <ScrollArea className="h-[300px] w-full rounded-md border p-4">
            {tripData.hotelDetails.map((hotel, index) => (
              <div key={index} className="mb-6 last:mb-0">
                ...
                {index < tripData.hotelDetails.length - 1 && (
          ...
          <Accordion type="single" collapsible className="w-full">
            {tripData.itineraryPlan.map((day, index) => (
              <AccordionItem key={index} value={`day-${day.day}`}>
                <AccordionTrigger>
                  Day {day.day} - {day.best_time_to_visit}
                </AccordionTrigger>
                <AccordionContent>
