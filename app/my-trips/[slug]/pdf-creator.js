...
export async function generateStyledPDF(tripData) {
  ...
  // Draw Trip Name
  drawText(tripData.tripName, 50, y, {
    size: 18,
  ...
  drawText(tripData.tripDescription, 50, y, {
    font,
    size: 14,
  ...
  drawText(`Avg. Budget: ${tripData.averageBudgetPerPerson}`, 300, y, {
    size: 12,
  });
  y -= 30;
  ...
  tripData.hotelDetails.forEach((hotel) => {
    ...
    drawText(`Address: ${hotel["Hotel address"]}`, 50, y - 15, {
      font,
      size: 10,
    });
    drawText(`Price: ${hotel.Price} | Rating: ${hotel.Rating}`, 50, y - 30, {
      font,
      size: 10,
    });
    drawText(`Description: ${hotel.description}`, 50, y - 45, {
      font,
      size: 10,
    });
    y -= 60;
  ...
  tripData.itineraryPlan.forEach((day) => {
    ...
    day.places.forEach((place) => {
      ...
      drawText(
        `Ticket: ${place["ticket pricing"]} | Rating: ${place.rating}`,
        60,
        y - 15,
        { size: 10 }
      );
      drawText(`Travel Time: ${place["time travel"]}`, 60, y - 30, {
        size: 10,
      });
      y -= 50;
    });
